...
Sphere::Sphere(glm::vec2 position, glm::vec2 velocity, float mass, float radius, glm::vec4 color) : RigidBody(SPHERE, position, velocity, 0.0f, mass)
{
	///set variables
	m_radius = radius;
	m_color = color;
...
{
	///checks for if the sphere is colliding with something
